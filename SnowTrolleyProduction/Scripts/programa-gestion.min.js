...
function onEnsambleChange() {
    ...
    $.get(PG.urls.getLadosPorEnsamble, { ensamble: id }, function (r) {
        ...
        if (r.lados.length > 1) {
            $cont.append('<div style="width:100%;margin-top:4px;font-size:.75rem;color:#6b7280;"><i class="fas fa-info-circle"></i> Se cargar�n todos los lados autom�ticamente.</div>');
        }
    });
}
...
function buildMachineLayout(cabezalNum, suffix, trolleys, acomodo, maqId, maqNombre, maquinas) {
    ...
    if (!maquinas.length) html += '<option value="0">Sin m�quinas</option>';
    html += '</select></div>';
    ...
    html += '<div class="setup-machine-rect"><i class="fas fa-microchip"></i> ' + (maqNombre || 'M�quina') + '</div>';
    html += '</div>';
    html += '<div class="setup-slots-row">';
...
function abrirPreview(data, totalOriginal) {
        ...
        columns: [
            {
                ...
                template: function (d) {
                    return d.EsDuplicado
                        ? '<i class="fas fa-exclamation-triangle" style="color:#ef4444;font-size:16px;" title="' + (d.RazonRechazo || '') + '"></i>'
            ...
            { field: 'Comentarios',       title: 'Notas',     width: 180 },
            {
                ...
                template: '<a class="k-button k-button-sm pg-preview-del" href="\\#" title="Quitar" style="color:\\#ef4444;min-width:0;padding:4px 8px;"><i class="fas fa-trash-alt"></i></a>'
            }
        ]
    });
...
function actualizarConteoPreview() {
    ...
    var dupl = 0;
    for (var i = 0; i < n; i++) {
        if (all[i].EsDuplicado) dupl++;
    }
    var ok = n - dupl;

    $('#previewCountLabel').html(
        '<i class="fas fa-list-ol"></i> <b>' + n + '</b> registros totales &nbsp;|&nbsp; ' +
        '<i class="fas fa-check-circle" style="color:#22c55e;"></i> <b>' + ok + '</b> v�lidos'
    );

    if (dupl > 0) {
        $('#previewDuplicateLabel').html(
            '<i class="fas fa-exclamation-triangle"></i> <b>' + dupl + '</b> duplicados detectados (ser�n omitidos al guardar)'
        ).show();
...
$(function () {
    ...
    $('#btnSubirExcel').on('click',    function (e) {
        e.preventDefault();
        $('#archivoExcelInput').val('');
        $('#excelUploadProgress').hide();
        $('#wndExcel').data('kendoWindow').center().open();
    });
    ...
    $('#btnPreviewExcel').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                abrirPreview(r.data, r.totalOriginal || r.data.length);
            },
            error: function () {
                $('#excelUploadProgress').hide();
                $btn.prop('disabled', false).html('<i class="fas fa-search"></i> Vista Previa');
                swalErr('Error', 'No se pudo procesar el archivo.');
            }
        });
    });
    ...
    $('#btnEliminarDuplicados').on('click', function () {
        ...
        var toRemove = ds.data().filter(function (x) { return x.EsDuplicado; });
        toRemove.forEach(function (item) { ds.remove(item); });
        actualizarConteoPreview();
    });
    ...
    $('#btnConfirmarCarga').on('click', function () {
        ...
        if (!items.length) {
            swalWarn('Sin datos v�lidos', 'No quedan registros v�lidos para guardar. Todos est�n marcados como duplicados.');
            return;
        }

        var mensaje = 'Se guardar�n <b>' + items.length + '</b> registros nuevos.';
        if (duplicados > 0) {
            mensaje += '<br/><small style="color:#ef4444;">Se omitir�n ' + duplicados + ' duplicados.</small>';
        }
        ...
        Swal.fire({
            ...
            cancelButtonColor: '#9aa0b8',
            confirmButtonText: '<i class="fas fa-check"></i> S�, guardar',
            ...
            $.ajax({
                url: PG.urls.guardarPreview,
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(items),
                success: function (r) {
                    if (r && r.success) {
                        swalOk('��xito!', 'Se guardaron ' + r.count + ' programas correctamente.', { timer: 2500, showConfirmButton: false });
                        $('#wndPreview').data('kendoWindow').close();
                        cargarKanban();
                    } else {
                        swalErr('Error', r && r.message ? r.message : 'Error desconocido');
                    }
                },
                error: function () { swalErr('Error', 'Error de conexi�n al servidor.'); }
    ...
    $(document).on('click', '.btn-card-del', function (e) {
        ...
        Swal.fire({ title: '�Eliminar?', text: 'Esta acci�n no se puede deshacer.', type: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#9aa0b8', confirmButtonText: 'S�, eliminar', cancelButtonText: 'Cancelar' })
        .then(function (result) {
            if (!result.value) return;
            ...
            }).fail(function (xhr) { swalErr('Error', 'Error de conexi�n: ' + xhr.status); });
        });
    });
});
