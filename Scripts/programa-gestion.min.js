...
function actualizarConteoPreview() {
    ...
    $('#previewCountLabel').html(
        ...
        '<i class="fas fa-check-circle" style="color:#22c55e;"></i> <b>' + ok + '</b> válidos'
    );
    
    if (dupl > 0) {
        $('#previewDuplicateLabel').html(
            '<i class="fas fa-exclamation-triangle"></i> <b>' + dupl + '</b> duplicados detectados (serán omitidos al guardar)'
        ).show();
...
$(function () {
    ...
    $('#btnConfirmarCarga').on('click', function () {
        ...
        if (!items.length) { 
            swalWarn('Sin datos válidos', 'No quedan registros válidos para guardar. Todos están marcados como duplicados.'); 
            return; 
        }
        
        var mensaje = 'Se guardarán <b>' + items.length + '</b> registros nuevos.';
        if (duplicados > 0) {
            mensaje += '<br/><small style="color:#ef4444;">Se omitirán ' + duplicados + ' duplicados.</small>';
        }
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                success: function (r) {
                    if (r && r.success) {
                        swalOk('¡Éxito!', 'Se guardaron ' + r.count + ' programas correctamente.', { timer: 2500, showConfirmButton: false });
                ...
                error: function () { swalErr('Error', 'Error de conexión al servidor.'); }
